...
<img src={image} alt="SaasRock v0.6 - Usage-based, One-time, and Multi-currency pricing" />
...
- [Prisma](https://www.prisma.io/): Next-generation object–relational mapper
...
![August wrap up](https://cdn-images-1.medium.com/max/800/1*i4IWw0Vo1aDCgn4muMyhmw.png)
...
![New database models](https://cdn-images-1.medium.com/max/800/1*Z8GLbK7yYn64e-lxUpiVKA.png)
...
![FLAT_RATE](https://cdn-images-1.medium.com/max/800/1*68pLqsifIIN48iH7TKnERQ.png)

### PER_SEAT - /pricing?model=1

![PER_SEAT](https://cdn-images-1.medium.com/max/800/1*3sUSmq4p1QhDTmZTciemLg.png)

### USAGE_BASED - /pricing?model=2

![USAGE_BASED](https://cdn-images-1.medium.com/max/800/1*k9YSplZjGzOof-eWrG4Xgw.png)

### FLAT_RATE_PLUS_USAGE_BASED - /pricing?model=3

![FLAT_RATE_PLUS_USAGE_BASED](https://cdn-images-1.medium.com/max/800/1*bLppDIRYgori2ICpeAO6-g.png)

### ONE_TIME - /pricing?model=4

![ONE_TIME](https://cdn-images-1.medium.com/max/800/1*8DeoQ0x-wYrZpgwPe2Hz7w.png)

Once you're somewhat happy with the plans, devs can easily generate them from `/admin/setup/pricing`:

![/admin/setup/pricing](https://cdn-images-1.medium.com/max/800/1*5d3onscvYFdU6P2Jc8cvDA.png)

But you can also create them one by one at `/admin/setup/pricing/new`:

![New pricing plan](https://cdn-images-1.medium.com/max/800/1*vDgRFNtJLpzQ8wLDtsSV6g.png)
...
![Plan Features & Limits](https://cdn-images-1.medium.com/max/800/1*uA9GGFJ-VzYOyfusrvaCiQ.png)
...
![Flat rate + Usage-based](https://cdn-images-1.medium.com/max/800/1*jQuEvVchHBiFtAzwmEM_xQ.png)

Our **Starter** product, should look like this, note that I'm supporting **_USD_** and **_MXN_** currencies.

![Starter Plan - Stripe Dashboard](https://cdn-images-1.medium.com/max/800/1*aE3w14Gvg9FCKTu1hBFaYw.png)
...
![Starter Plan - Stripe Checkout Session](https://cdn-images-1.medium.com/max/800/1*naZoGKxqiYeJEhIlkoAULQ.png)

The Stripe subscription will be **Active**, have two **Subscription Items**, and have an **Upcoming Invoice**.

![Customer Subscription - Stripe Dashboard](https://cdn-images-1.medium.com/max/800/1*SXzc0l_eXIS0adtE2m_bqw.png)
...
![GET employees API call - VS Code Thunder Client](https://cdn-images-1.medium.com/max/800/1*vSO-1Mca0-Pj7Drpynnhzg.png)

The Upcoming invoice will still be $199 because API calls are free until we reach 10 units:

![Stripe Upcoming Invoice](https://cdn-images-1.medium.com/max/800/1*pmQtaSquzhNSl8OBJLlH9A.png)

After reaching 11 units _(API calls)_, we'll reach the 2nd tier, where each unit costs $0.04 and there's a flat fee of $10:

![$199 + (0.04 x 1) + $10 = $209.44](https://cdn-images-1.medium.com/max/800/1*DMDePW39B7PkZmGVvdllLQ.png)

And our customer can see the **Upcoming Invoice** at `/app/:tenant/settings/subscription`:

![Upcoming Invoice](https://cdn-images-1.medium.com/max/800/1*kqmsirwffLCHJW6_f7s3Rg.png)

Now let's get to the 3rd tier, with 21 units _(API calls)_, where units cost $0.02 _(cheaper)_ but there's a flat fee of $20:

![$199 + (0.02 x 21) + $20 = $219.42](https://cdn-images-1.medium.com/max/800/1*zcLk8VH1eqy1SUe4VN7shw.png)
...
![Upcoming Invoice - Graduated](https://cdn-images-1.medium.com/max/800/1*jb5SSSUnVFInJ4hwqhGnKg.png)
...
![reportUsage(tenant, unit)](https://cdn-images-1.medium.com/max/800/1*DKX0X95kXHZh4Xz-Q9QBHw.png)
...
- Add-ons
- Any digital good: _ebooks, courses, videos…_
...
![One-time plan](https://cdn-images-1.medium.com/max/800/1*-DgIaW2M4IDrBhFKnu0hyQ.png)
...
![3 recurring + 1 one-time plans](https://cdn-images-1.medium.com/max/800/1*ZLOVJHG17mSEWTvfHN7Nww.png)

Finally, our subscription looks like this:

![All-access + Starter plans](https://cdn-images-1.medium.com/max/800/1*4gNnnaqcICQ75KG3nhAO9A.png)
...
![app/utils/db/appConfiguration.db.server.ts](https://cdn-images-1.medium.com/max/800/1*CnfEka92UhSpvixoAeJoQA.png)

![/register](https://cdn-images-1.medium.com/max/800/1*BU9NhuxrunoOTiXz3OOqzA.jpeg)
...
![Tenant Subscription](https://cdn-images-1.medium.com/max/800/1*8br7WpVeHeR77HxOJQjAFw.png)
...
![/pricing](https://cdn-images-1.medium.com/max/800/1*ykuqCqkvsK0EysfNSOa7HA.png)

### Stripe Checkout

![Stripe Checkout](https://cdn-images-1.medium.com/max/800/1*plAmWXjLxZhVCc7cBwHJOA.png)

### Checkout Success page

![/pricing/:session_id/success](https://cdn-images-1.medium.com/max/800/1*aZB6hUTUAl2RoWVUUB6ktQ.png)
...
![Set up Account Email](https://cdn-images-1.medium.com/max/800/1*277WFhDQl2NcKMD3L48FeA.png)
...
![Bought plans already added to a tenant/account](https://cdn-images-1.medium.com/max/800/1*StXZyHuKCDrsuc1wmJzAEg.png)

If you want to enable/disable one sign-up flow, just update the following flags:

- **required**: _true_ → redirects user to `/subscribe/:tenant`
- **allowSubscribeBeforeSignUp**: _false_ → buttons at `/pricing` are hidden
- **allowSignUpBeforeSubscribe**: _false_ → `/register` redirects to `/pricing`
...
![locales/…/translations.json](https://cdn-images-1.medium.com/max/1600/1*mChQWDaNbqGEMA8puXC3nQ.png)
...
![English & Spanish - Pricing page](https://cdn-images-1.medium.com/max/1600/1*_cKdOzl3opvH8x2HjkC5fg.png)
...
You could imagine that if I create 10 _employees_ from the _API_, that would be 20 units 😂, but I'll do it just for demonstration purposes - I'm using VOLUME tiers.
...
![SaasRock community feedback](https://cdn-images-1.medium.com/max/800/1*kgrGwsqmxLGkhAY0BnZxTQ.png)
